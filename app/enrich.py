...
def _cut_at_repetition(words: list[str]) -> int:
    ...
    for index, word in enumerate(words):
        key = _fold(word)
        if len(key) >= 4 and any(key.startswith(other) or other.startswith(key) for other in seen):
            return index
        if len(key) >= 4:
            seen.append(key)
    return len(words)
...
def categorize(text: str, *, rules: list[tuple[str, tuple[str, ...]]] | None = None) -> str:
